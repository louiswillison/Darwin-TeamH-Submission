...
class FeatureExtractor:
    def __init__(
        self,
        use_subset: bool,
        output_path="features_CoughCOSpeechGE.csv",
        batch_size=1000,
        verbose: bool = False,
        feature_type="COUGH_MFCC",
    ):
    ...
    def extract_audio_files(self, web=False, ios=True, android=True):
        ...
        if web:
            paths.append(
                os.path.join(
                    metadata_dir, "results_raw_20210426_lan_yamnet_web_noloc.csv"
                )
            )
        if ios:
            paths.append(
                os.path.join(
                    metadata_dir, "results_raw_20210426_lan_yamnet_ios_noloc.csv"
                )
            )
        if android:
            paths.append(
                os.path.join(
                    metadata_dir, "results_raw_20210426_lan_yamnet_android_noloc.csv"
                )
            )
    ...
    def process_file(self, file_path):
        data_batch = []
        row_count = sum(1 for _ in open(file_path, mode="r"))

        with open(file_path, mode="r") as file:
            reader = csv.reader(file, delimiter=";")
            header = next(reader)

            for row in tqdm(reader, leave=False, total=row_count):
    ...
    def process_row(self, row):
        ...
        if (
            self.feature_type == "COUGH_MFCC"
            or self.feature_type == "COUGH_GeMAPS"
            or self.feature_type == "COUGH_COMPARE"
        ):
            path = os.path.join(
                "../data/covid19_data_0426", folder_id, folder_name, cough_file
            ).replace("\\", "/")
            if row[16] != "c":
                return None
        else:
            path = os.path.join(
                "../data/covid19_data_0426", folder_id, folder_name, speech_file
            ).replace("\\", "/")
            if row[15] != "v":
                return None
    ...
    def extract_features(self, audio_path, mode=None):
        ...
        if (self.feature_type == "COUGH_MFCC") or (mode == "MFCC"):
            ...
            if time_steps < self.target_time_steps:
                ...
                features = np.pad(
                    features, pad_width=((0, 0), (0, pad_width)), mode="constant"
                )
            else:
        ...
        if (
            self.feature_type == "COUGH_COMPARE"
            or self.feature_type == "SPEECH_COMPARE"
        ) or (mode == "ComParE"):
        ...
        if (
            self.feature_type == "COUGH_GeMAPS" or self.feature_type == "SPEECH_GeMAPS"
        ) or (mode == "GeMAPS"):
            smile = opensmile.Smile(
...
def extract_features(feature_type, output_path):
    extractor = FeatureExtractor(
        use_subset=False,
        verbose=True,
        feature_type=feature_type,
        output_path=output_path,
    )  # COUGH_MFCC, COUGH_GeMAPS, COUGH_COMPARE, SPEECH_COMPARE, SPEECH_GeMAPS COUGHGEspeechGE COUGHGEspeechCO COUGHCOspeechCO COUGHCOspeechGE
